...
use App\Rules\ArquivoEnviadoAnalista;
...
class EnviarDocumentosAnalista extends Component
{   
    ...
    public function mount(Requerimento $requerimento)
    {
        ...
        $this->documentos = $this->requerimento->documentos_analista()->withPivot('status', 'comentario')->get();
    ...
    {
        return view('livewire.enviar-documentos-analista');
    }
    ...
    public function updated($propertyName, $value)
    {
        ...
        $documento = $this->requerimento->documentos_analista()->withPivot('status')->where('documento_id', $id)->first();
        if ($documento->pivot->status == AnalistaChecklist::STATUS_ENUM['nao_enviado'] || $documento->status == \App\Models\AnalistaChecklist::STATUS_ENUM['recusado'] || ($this->requerimento->status == $this->requerimentoStatus['documentos_requeridos'] || $documento->status == \App\Models\AnalistaChecklist::STATUS_ENUM['enviado'])) {
            ...
            $this->requerimento->documentos_analista()->updateExistingPivot($id, [
                'caminho' => $value->store("documentos_analista/requerimentos/{$this->requerimento->id}"),
                'status' => AnalistaChecklist::STATUS_ENUM['enviado'],
            ]);
            $this->dispatchBrowserEvent('swal:fire', [
                'icon' => 'success',
    ...
    private function rulesSubmit()
    {
        ...
        foreach ($this->documentos->pluck('id') as $key) {
            $rules['arquivos.' . $key] = [new ArquivoEnviadoAnalista($this->requerimento, $key)];
        }
